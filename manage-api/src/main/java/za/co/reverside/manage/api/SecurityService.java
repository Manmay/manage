...
public class SecurityService {
	...
	@RequestMapping(value="validate/{token}", method= GET)
	...
	public ResponseEntity<String> login(@RequestParam(value="code", required = false) String authorizationCode,
		...
		if(error!=null){
			...
			headers.add("Location", "http://localhost:8080/logout.html?message="+error);
        ...
        if(!googleUser.getEmail().endsWith("reverside.co.za")){
			...
			MultiValueMap<String, String> headers = new LinkedMultiValueMap<String, String>();
			headers.add("Location", "http://localhost:8080/logout.html?message=Please login with your reverside credentials");
	...
	public ResponseEntity<String>  logout(@CookieValue("token") String token){
		...
		MultiValueMap<String, String> headers = new LinkedMultiValueMap<String, String>();
		headers.add("Location", "http://localhost:8080/logout.html");
